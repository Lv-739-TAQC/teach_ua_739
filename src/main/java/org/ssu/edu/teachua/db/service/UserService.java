...
public class UserService {
    ...
    public static List<User> getUsersByEmailAndPassword(String email, String password) throws DBException, EntityException {
        return new UserDAOImpl().findElementsBySQlRequest(
                DBManager.getInstance().getConnection(), SQL_FIND_USER_BY_EMAIL_AND_PASSWORD, false, email, password
        );
    }

    public static List<User> getUsersByEmail(String email) throws DBException, EntityException {
        return new UserDAOImpl().findElementsBySQlRequest(
                DBManager.getInstance().getConnection(), SQL_FIND_USER_BY_EMAIL, true, email
        );
    }
}
